...
function reinstantiateTiles() {
	...
	const rootUrl = new URL(
		`${baseUrl}/v1/3dtiles/root.json?key=${params.gmapsAccessToken}`
	);
	console.log("DBS root URL: ", rootUrl.toString());
	let sessionToken = null;

	tiles = new TilesRenderer(rootUrl.toString());

	tiles.preprocessURL = (uri) => {
		// If the URI has a session token, cache it
		if (uri.includes("session=")) {
			sessionToken = uri.split("session=")[1].split("&")[0];
			console.log("DBS Saving session token: ", sessionToken);
		}

		// If this is the root tile, just return the uri
		if (uri.includes("root.json")) {
			return uri;
		}

		// A complete, pre-processed URL will have:
		// - the https part of the google tileset
		// - the session token
		// - the key
		//
		// Some URIs will have the https part of the google tileset, but not the
		// session token. In this case, we add the session token.
		// Some URIs will have the https part of the google tileset, but not the
		// key. In this case, we add the key.
		// Whichever other case, we will construct the URL as in the steps at the start.

		let uriWithHTTPPart = uri;
		if (!uri.startsWith("https://")) {
			uriWithHTTPPart = `${baseUrl}${uri}`;
		}

		const url = new URL(uriWithHTTPPart);
		if (!url.searchParams.has("session")) {
			url.searchParams.append("session", sessionToken);
		}
		if (!url.searchParams.has("key")) {
			url.searchParams.append("key", params.gmapsAccessToken);
		}

		console.log("Preprocess URL url: ", url.toString());
		return url.toString();
	};
